...
namespace PersistanceMap.Test.Integration
{
    ...
    public class DeleteTests : TestBase
    {
        ...
        {
            var connection = new DatabaseConnection(new MockSqlContextProvider(ConnectionString, "DELETE from Employees"));
        ...
        {
            var connection = new DatabaseConnection(new MockSqlContextProvider(ConnectionString, "DELETE from Employees where (Employees.EmployeeID = 1)"));
            using (var context = connection.Open())
            {
